<script setup>
    ...
    //===========Change Password=========
    const changeFormPassword = useForm({
        current_password: '',
        password: '',
    ...
    const updatePassword = () =>{
       if(changeFormPassword.current_password.length === 0){
             toast.warning('Enter Your Current Password');
       }
       else if(changeFormPassword.password.length === 0) {
              toast.warning('Enter Your New Password');
       }
       else if(changeFormPassword.password_confirmation.length === 0){
             toast.warning('Enter Your Confirm Password');
       }
       else{
          changeFormPassword.post('/profile/password',{
            onSuccess: (page) =>{
                 page.props.flash.success && toast.success(page.props.flash.success);
                page.props.flash.error && toast.error(page.props.flash.error);
            },
            ...
            <form @submit.prevent="updateProfile">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" id="cus_postcode" class="w-full p-2 border rounded-md" v-model="form.cus_postcode">
                    </div>
                    <div class="mb-4">
                        <label for="cus_country" class="block text-gray-700 font-bold mb-2">Customer Country</label>
                        <input type="text" id="cus_country" class="w-full p-2 border rounded-md" v-model="form.cus_country">
                    </div>
                    <div class="mb-4">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" id="ship_state" class="w-full p-2 border rounded-md" v-model="form.ship_state">
                    </div>
                    <div class="mb-4">
                        <label for="ship_postcode" class="block text-gray-700 font-bold mb-2">Shipping Postcode</label>
                        <input type="text" id="ship_postcode" class="w-full p-2 border rounded-md" v-model="form.ship_postcode">
                    </div>
                    <div class="mb-4">
                        <label for="ship_country" class="block text-gray-700 font-bold mb-2">Shipping Country</label>
                        <input type="text" id="ship_country" class="w-full p-2 border rounded-md" v-model="form.ship_country">
                    </div>
                    <div class="mb-4">
                        <label for="ship_phone" class="block text-gray-700 font-bold mb-2">Shipping Phone</label>
                        <input type="text" id="ship_phone" class="w-full p-2 border rounded-md" v-model="form.ship_phone">
                    </div>
                </div>
            ...
            <form @submit.prevent="updatePassword">
                <div class="mb-4">
                    <label for="current-password" class="block text-gray-700 font-bold mb-2">Current
                        Password</label>
                    <input type="password" id="current-password" class="w-full p-2 border rounded-md" v-model="changeFormPassword.current_password">
                </div>

                <div class="mb-4">
                    <label for="new-password" class="block text-gray-700 font-bold mb-2">New Password</label>
                    <input type="password" id="new-password" class="w-full p-2 border rounded-md" v-model="changeFormPassword.password">
                </div>

                <div class="mb-4">
                    <label for="confirm-password" class="block text-gray-700 font-bold mb-2">Confirm New
                        Password</label>
                    <input type="password" id="confirm-password" class="w-full p-2 border rounded-md" v-model="changeFormPassword.password_confirmation">
                </div>
